...
async def list_resources(session: AsyncSession = Depends(get_db_session)) -> list[ResourceRead]:
...
async def create_resource(
    payload: ResourceCreate, session: AsyncSession = Depends(get_db_session)
) -> ResourceRead:
